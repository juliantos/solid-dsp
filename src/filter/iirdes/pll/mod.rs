...
/// ```
pub fn active_lag(bandwidth: f64, damping_factor: f64, loop_gain: f64) -> Result<(Vec<f64>, Vec<f64>), Box<dyn Error>> {
    if bandwidth <= 0.0 {
        return Err(Box::new(IirdesError(IirdesErrorCode::InvalidBandwidth)))
    } else if damping_factor <= 0.0 {
        return Err(Box::new(IirdesError(IirdesErrorCode::InvalidDampingFactor)))
    } else if loop_gain <= 0.0 {
        return Err(Box::new(IirdesError(IirdesErrorCode::InvalidGain)))
    }
...
/// assert_eq!(filter.0[1], 4000.0);
/// ```
pub fn active_proportional_integral(bandwidth: f64, damping_factor: f64, loop_gain: f64) -> Result<(Vec<f64>, Vec<f64>), Box<dyn Error>> {
    if bandwidth <= 0.0 {
        return Err(Box::new(IirdesError(IirdesErrorCode::InvalidBandwidth)))
    } else if damping_factor <= 0.0 {
        return Err(Box::new(IirdesError(IirdesErrorCode::InvalidDampingFactor)))
