...
impl<C: Copy + Sum + Num + Neg<Output = C>> AutoCorrelator<C> {
    ...
    where
        Complex<C>: Real<Output = f64>,
    {
        self.window.push(sample);
        self.window_with_delay.push(sample.conj());

        let e2 = (sample * sample.conj()).real();
        self.energy_sum -= self.energy_buffer[self.energy_index];
        self.energy_sum += e2;
    ...
    pub fn execute(&self) -> Complex<C> {
        self.window
            .to_vec()
            .iter()
            .zip(self.window_with_delay.to_vec().iter())
            .map(|(&x, &y)| x * y)
            .sum()
    }
...
impl<C> fmt::Display for AutoCorrelator<C> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "AutoCorrelator<{}> [Size={}] [Delay={}] [Energy={}]",
            typename, self.window_size, self.delay, self.energy_sum
        )
    }
}
