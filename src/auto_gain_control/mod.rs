...
impl fmt::Display for AGCError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let error_code = match self.0 {
            AGCErrorCode::BandwidthOutOfRange => {
                self.1.to_string() + " Bandwidth not in range [0, 1]"
            }
            AGCErrorCode::SignalLevelOutOfRange => {
                self.1.to_string() + " Level is too low (0, inf)"
            }
            AGCErrorCode::GainBelowThreshold => {
                self.1.to_string() + " Gain is below Threshold (0, inf)"
            }
            AGCErrorCode::ScaleBelowThreshold => {
                self.1.to_string() + " Scale is below Threshold (0, inf)"
            }
...
impl AGC {
    ...
    /// ```
    pub fn execute<
        T: Mul<f64, Output = T> + Mul<T, Output = T> + Conj<Output = T> + Real<Output = f64> + Copy,
    >(
        &mut self,
        input: T,
    ) -> T {
        ...
        if self.squelch_mode == SquelchMode::ENABLED {
            return input;
        }
        out * self.scale
    }
...
impl fmt::Display for AGC {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "AGC [Gain={:.5}] [Scale={:.5}] [Bandwidth={:.5}] [Alpha={:.5}] [Energy={:.5}]",
