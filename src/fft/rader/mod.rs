use super::super::fft::{FFT, FFTDirection, FFTFlags, FFTType, FFTMethod, DataUnion, Rader};
...
pub fn create_rader_plan(nfft: usize, direction: FFTDirection, flags: FFTFlags) -> FFT {
    ...
    for i in 0..(nfft - 1) {
        let exponent = Complex::from_polar(1.0, d * 2.0 * std::f64::consts::PI * (seq[i] as f64) / (nfft as f64));
    ...
    let freq_domain_buffer = fft.execute(&time_domain_buffer).unwrap_or(vec![]);
    ...
    FFT {
        ...
        fft_flags: flags,
        data: DataUnion { rader: ManuallyDrop::new(Rader {
            seq: seq,
...
pub fn rader_execute(fft: &FFT, input: &[Complex<f64>]) -> Result<Vec<Complex<f64>>, Box<dyn std::error::Error>> {
    ...
    freq_domain_buffer = unsafe { freq_domain_buffer.iter().zip(fft.data.rader.dft.iter()).map(|(&x, &y)| x * y).collect() };
    ...
    for i in 0..(fft.nfft - 1) {
        ...
        output[k] = (time_domain_buffer[i] / (fft.nfft - 1) as f64) + input[0];
    }
