use super::super::fft::{FFT, FFTDirection, FFTFlags, FFTType, FFTMethod, DataUnion, MixedRadix};
...
fn estimate_mixed_radix(nfft: usize) -> usize {
    ...
    if num_factors_2 > 0 {
        if nfft % 16 == 0 { return 16 }
        else if nfft % 8 == 0 { return 8 }
        else if nfft % 4 == 0 { return 4 }
        else { return 2 }
    }
...
pub fn create_mixed_radix_plan(nfft: usize, direction: FFTDirection, flags: FFTFlags) -> FFT {
    ...
    if q == 0 { panic!("Tried To Create a Mixed Radix Plan with a Prime nFFT {nfft}!") };
    if nfft % q != 0 { panic!("Tried To Create a Mixed Radix Plan Where nFFT: {nfft} is not divisible by {q}") };
    ...
    FFT {
        ...
        fft_flags: flags,
        data: DataUnion { mixed_radix: ManuallyDrop::new(MixedRadix {
            q: q,
            p: p,
...
pub fn mixed_radix_execute(fft: &FFT, input: &[Complex<f64>]) -> Result<Vec<Complex<f64>>, Box<dyn std::error::Error>> {
